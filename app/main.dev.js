...
const directories = [`${homedir}/.lumanexwallet`, `${homedir}/.lumanexwallet/logs`];
...
const daemonLogFile = path.resolve(directories[1], 'lumanex.log');
  ...
  prepend: (defaultActions, params) => [
    {
      ...
      click: () => {
        shell.openExternal(
          `${Config.explorerBaseURL}${encodeURIComponent(params.selectionText)}#blockchain_transaction`
        );
      }
    },
    {
...
app.on('ready', async () => {
  ...
  mainWindow = new BrowserWindow({
    title: `Lumanex Wallet v${version.toString()}`,
    useContentSize: true,
    width: 1200,
    height: 625,
    minWidth: 1200,
